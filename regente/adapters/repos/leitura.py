...
CLI_FLAGS_QUE_ESCREVEM: frozenset[str] = frozenset({
...
CLI_LEITURA: frozenset[tuple[str, str | None]] = frozenset({
    ("repo", "list"), ("repo", "view"),
...
def git_e_leitura(args: list[str] | tuple[str, ...]) -> tuple[bool, str]:
    ...
    proibidas = [a for a in resto if a in GIT_FLAGS_DE_ESCRITA]
    if proibidas:
        return False, f"flag de escrita: {', '.join(proibidas)}"

    if cmd in GIT_SEMPRE_LEITURA:
        return True, ""
...
def cli_e_leitura(args: list[str] | tuple[str, ...]) -> tuple[bool, str]:
    ...
    if cmd == "api":
        if ("api", None) not in CLI_LEITURA:
            return False, "'api' nao esta liberado"
        for i, a in enumerate(args):
            base = a.split("=", 1)[0]
            if base not in CLI_FLAGS_QUE_ESCREVEM:
                continue
            if base in ("--method", "-X"):
